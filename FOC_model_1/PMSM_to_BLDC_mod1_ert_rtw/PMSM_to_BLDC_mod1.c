...
/* Forward declaration for local functions */
static void PMSM_to_BLDC_m_SystemCore_setup(stm32cube_blocks_SPIControl_b_T *obj);

/* Forward declaration for local functions */
static void PMSM_to_BL_SystemCore_setup_oxg(stm32cube_blocks_SPIControl_b_T *obj);
static void PMSM_to_SPIDrvBlock_setupImpl_o(stm32cube_blocks_SPIControlle_T *obj);
static void PMSM_to_B_SystemCore_setup_oxgk(stm32cube_blocks_SPIControl_b_T *obj);
static void PMSM_to__SystemCore_setup_oxgkf(stm32cube_blocks_SPIControl_b_T *obj);
...
void PMSM_to_BLDC_mod1_step(void)
{
  ...
  if (PMSM_to_BLDC_mod1_DW.is_active_c3_PMSM_to_BLDC_mod1 == 0) {
    ...
    switch (PMSM_to_BLDC_mod1_DW.is_c3_PMSM_to_BLDC_mod1) {
     ...
     case PMSM_to_BLDC_mod1_IN_SPI_init4:
      PMSM_to_BLDC_mod1_B.Ctrl_flag2 = 0.0;
      if ((PMSM_to_BLDC_mod1_B.Constant2 == 1.0) &&
          (PMSM_to_BLDC_mod1_B.Constant3 == 1.0)) {
        PMSM_to_BLDC_mod1_DW.temporalCounter_i1 = 0U;
        PMSM_to_BLDC_mod1_DW.is_c3_PMSM_to_BLDC_mod1 =
  ...
  if (PMSM_to_BLDC_mod1_B.AND1) {
    ...
    status = MW_STM32_SPI_SetFormat(PMSM_to_BLDC_mod1_DW.obj_gk.MW_SPI_HANDLE,
  ...
  if (PMSM_to_BLDC_mod1_B.AND) {
    ...
    PMSM_to_BLDC_mod1_B.BitwiseOR_e = (uint16_T)(status | y_n |
    ...
    status = MW_STM32_SPI_SetFormat(PMSM_to_BLDC_mod1_DW.obj_g.MW_SPI_HANDLE, 2U,
      3840U, MW_SPI_MODE_3);
    if (status == 0) {
      MW_SPI_Read_Databits(PMSM_to_BLDC_mod1_DW.obj_g.MW_SPI_HANDLE, &status, 1U,
                           1, 1, 1U);
      LL_GPIO_SetOutputPin(GPIOC, 16384U);
    }
  ...
  if (PMSM_to_BLDC_mod1_B.spi_write1 > 0.0) {
    ...
    status = MW_STM32_SPI_SetFormat(PMSM_to_BLDC_mod1_DW.obj.MW_SPI_HANDLE, 2U,
      3840U, MW_SPI_MODE_3);
    if (status == 0) {
      MW_SPI_Read_Databits(PMSM_to_BLDC_mod1_DW.obj.MW_SPI_HANDLE, &tmp_0[0],
                           15U, 1, 1, 1U);
      LL_GPIO_SetOutputPin(GPIOC, 16384U);
    }
    ...
    PMSM_to_BLDC_mod1_B.BitwiseOR_h = (uint16_T)(status | y_n |
  ...
  if (PMSM_to_BLDC_mod1_B.spi_write3 > 0.0) {
    ...
    /* MATLABSystem: '<S37>/SPI Transmit' */
    status = MW_STM32_SPI_SetFormat(PMSM_to_BLDC_mod1_DW.obj_k.MW_SPI_HANDLE, 2U,
      3840U, MW_SPI_MODE_3);
    if (status == 0) {
      LL_GPIO_ResetOutputPin(GPIOC, 16384U);
   ...
   */
  PMSM_to_BLDC_mod1_B.Compare = (PMSM_to_BLDC_mod1_B.SPIReceive_pna.SPIReceive ==
    6148);
...
void PMSM_to_BLDC_mod1_initialize(void)
{
  ...
  /* External mode info */
  PMSM_to_BLDC_mod1_M->Sizes.checksums[0] = (3135476383U);
  PMSM_to_BLDC_mod1_M->Sizes.checksums[1] = (250380925U);
  PMSM_to_BLDC_mod1_M->Sizes.checksums[2] = (2432057980U);
  PMSM_to_BLDC_mod1_M->Sizes.checksums[3] = (3970126717U);

  {
    ...
    systemRan[37] = (sysRanDType *)&PMSM_to_BLDC_mod1_DW.Subsystem1_SubsysRanBC;
  ...
  /* block I/O */
  {
    PMSM_to_BLDC_mod1_B.Constant2 = 1.0;
    PMSM_to_BLDC_mod1_B.Constant3 = 1.0;
  }
  ...
  PMSM_to__SystemCore_setup_oxgkf(&PMSM_to_BLDC_mod1_DW.obj);
  ...
  PMSM_to_SPIDrvBlock_setupImpl_o(&PMSM_to_BLDC_mod1_DW.obj_k);
