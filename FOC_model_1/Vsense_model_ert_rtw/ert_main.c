...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) Vsense_model_M->Timing.taskTime0;
  Vsense_model_step();
...
int main(int argc, char **argv)
{
  ...
  rtmSetErrorStatus(Vsense_model_M, 0);
  ...
  Vsense_model_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(Vsense_model_M->extModeInfo, &rtmGetTFinal
    (Vsense_model_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Vsense_model_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Vsense_model_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
