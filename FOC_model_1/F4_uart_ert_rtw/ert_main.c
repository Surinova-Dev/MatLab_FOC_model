...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) F4_uart_M->Timing.taskTime0;
  F4_uart_step();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(F4_uart_M->extModeInfo, &rtmGetTFinal(F4_uart_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(F4_uart_M, true);
    }
  }
